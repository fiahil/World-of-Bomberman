...
SkinChoose::SkinChoose(GameManager& game)
  : AMenu("menu/background/backgroundPersonnalization.jpg", "menu/background/backgroundPersonnalization.jpg",
	  3200.0f, -1.0f, 1600.0f, game),
    _model(Skin::LAST - 1),
    _rot(0.0f),
    _timerL(-1.0f),
    _timerR(-1.0f),
...
{
  this->_tags.push_back(new Tag("menu/tags/PersonalizationSkinNormal.png", "menu/tags/PersonalizationSkinHighlit.png",
				true, false, TokenMenu::LAST, 3500.0f, 0.0f, 1955.0f));
  this->_tags.push_back(new Tag("menu/tags/PersonalizationSkillNormal.png", "menu/tags/PersonalizationSkillHighlit.png",
				false, false, TokenMenu::LAST, 3500.0f, 0.0f, 2020.0f));
  this->_tags.push_back(new Tag("menu/tags/DoneNormal.png", "menu/tags/DoneHighlit.png",
				false, false, TokenMenu::SETTINGSCHOOSE, 3500.0f, 0.0f, 2085.0f));
}
...
{
  AMenu::initialize();
  this->_model.at(0) = gdl::Model::load(g_refModel[0]);
  this->_model.at(1) = gdl::Model::load(g_refModel[1]);
  this->_model.at(2) = gdl::Model::load(g_refModel[2]);
  this->_model.at(3) = gdl::Model::load(g_refModel[3]);
...
{
  this->_tags.at(0)->createText(g_refSkin[this->_indexSkin], 20, 700, 370);
  this->_tags.at(1)->createText(g_refSkill[this->_indexSkill], 20, 650, 435);
}
...
void		SkinChoose::update(gdl::GameClock const& clock, gdl::Input& input)
{
  ...
  this->_model.at(this->_indexSkin).update(clock);
  this->updateText();
...
void	SkinChoose::draw()
{
  ...
  if (this->_indexSkin < Skin::LAST - 1)
    this->_model.at(this->_indexSkin).draw();
  glPopMatrix();
  glPushMatrix();
