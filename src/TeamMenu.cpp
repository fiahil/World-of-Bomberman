...
TeamMenu::TeamMenu(GameManager& game, std::vector<Profile*>& pro)
  : AMenu("menu/background/backgroundTeam.jpg", "menu/background/backgroundTeam.jpg",
	  0.0f, -1.0f, 2400.0f, game),
    _profiles(pro),
    _nb(3, 1),
    _timersLR(2, -1.0f)
{
  this->_nb.at(2) = -1;
  this->_tags.push_back(new Tag("menu/tags/TeamPlayerNormal.png", "menu/tags/TeamPlayerHighlit.png",
				true, false, TokenMenu::TEAM, 429.0f, 0.0f, 2750.0f));
  this->_tags.push_back(new Tag("menu/tags/TeamNumberNormal.png", "menu/tags/TeamNumberHighlit.png",
				false, false, TokenMenu::TEAM, 429.0f, 0.0f, 2815.0f));
  this->_tags.push_back(new Tag("menu/tags/TeamMultiNormal.png", "menu/tags/TeamMultiHighlit.png",
				false, false, TokenMenu::TEAM, 429.0f, 0.0f, 2880.0f));
  this->_tags.push_back(new Tag("menu/tags/DoneNormal.png", "menu/tags/DoneHighlit.png",
				false, false, TokenMenu::MAP, 429.0f, 0.0f, 2945.0f));
}
...
void		TeamMenu::updateText() const
{
  ...
  ss << this->_nb.at(0);
  this->_tags.at(0)->createText(ss.str(), 20, 870, 365);
  ss.str("");
  ss.clear();
  ss << this->_nb.at(1);
  this->_tags.at(1)->createText(ss.str(), 20, 870, 430);
  ss.str("");
  ss.clear();
  ss << this->_nb.at(2);
  if (this->_profiles.size() > 1 && this->_gameManager._match._gameMode != GameMode::SOLO)
    this->_tags.at(2)->createText(this->_profiles.at(this->_nb.at(2))->getName(), 20, 855, 495);
  else
    this->_tags.at(2)->createText("", 20, 855, 495);
}
...
void		TeamMenu::changeNumber(gdl::GameClock const& clock, gdl::Input& input)
{
  if (clock.getTotalGameTime() >= this->_timersLR.at(0) &&
       ...
       JsManager::getMe()->isJsDown(JsMode::MENU, gdl::Keys::Left)))
    {
	  ...
	  this->_nb.at(this->_cursor) < 1)
	this->_nb.at(this->_cursor) = 1;
      else if (this->_gameManager._match._gameMode == GameMode::VERSUS && this->_nb.at(this->_cursor) < 0)
       ...
       JsManager::getMe()->isJsDown(JsMode::MENU, gdl::Keys::Right)))
    {
  	...
  	this->_nb.at(this->_cursor) = 5;
      this->_timersLR.at(1) = clock.getTotalGameTime() + 0.15f;
    }
}
...
void		TeamMenu::changeProfile(gdl::GameClock const& clock, gdl::Input& input)
{
  ...
  if (clock.getTotalGameTime() >= this->_timersLR.at(0) &&
       ...
       JsManager::getMe()->isJsDown(JsMode::MENU, gdl::Keys::Left)))
    {
      --this->_nb.at(2);
      if (this->_nb.at(2) < 0)
	this->_nb.at(2) = this->_profiles.size() - 1;
      while (this->_profiles.at(this->_nb.at(2)) == this->_gameManager._mainProfile)
	{
	  --this->_nb.at(2);
	  if (this->_nb.at(2) < 0)
	    this->_nb.at(2) = this->_profiles.size() - 1;
...
{
  if (this->_nb.at(2) < 0 && this->_profiles.size() > 1)
    {
      this->_nb.at(2) = 0;
  ...
  if (this->_curToken == TokenMenu::MAP)
    {
      this->_gameManager._nbPlayers = this->_nb.at(0);
      this->_gameManager._nbTeams = this->_nb.at(1);
      if (this->_profiles.size() > 1 && this->_gameManager._match._gameMode != GameMode::SOLO)
	this->_gameManager._secondProfile = this->_profiles.at(this->_nb.at(2));
    }
}
